...
class SessionHistoryLoader:
    ...
    def load_and_embed_sessions(self) -> bool:
        ...
        try:
            ...
            print(f"📝 Processing {len(files_to_process)} new/modified session files...")
    ...
    def _extract_session_metadata(self, file_path: str, content: str) -> Dict[str, Any]:
        ...
        if characters:
            metadata['characters_mentioned'] = ', '.join(characters)  # Convert list to string
        
        # Extract locations
        locations = self._extract_locations(content)
        if locations:
            metadata['locations_mentioned'] = ', '.join(locations)  # Convert list to string
    ...
    def _extract_character_names(self, content: str) -> List[str]:
        ...
        known_characters = ['Rose', 'Luvash', 'Arabelle', 'Meda', 'Duras', 'Strahd', 'Ireena', 'Ismark']
    ...
    def _extract_locations(self, content: str) -> List[str]:
        ...
        known_locations = [
            'Barovia', 'Vallaki', 'Krezk', 'Ravenloft', 'Castle Ravenloft',
            'Death House', 'Village of Barovia', 'Tser Pool', 'Old Bonegrinder',
            'Wizard of Wines', 'Yester Hill', 'Amber Temple', 'tavern', 'church'
        ]
    ...
    def _create_session_chunks(self, content: str, base_metadata: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    def _split_into_sections(self, content: str) -> List[Dict[str, str]]:
        ...
        # Split by headers
        lines = content.split('\n')
        current_section = {'type': 'overview', 'title': 'Session Summary', 'content': ''}
