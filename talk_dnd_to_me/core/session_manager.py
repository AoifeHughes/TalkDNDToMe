...
class SessionManager:
        ...
        try:
            ...
            if llm_client:
                try:
                    ...
                    markdown_content = self.generate_markdown_summary(session_entries, llm_client)
                    markdown_file = self.save_markdown_summary(session_id, markdown_content)
                    if markdown_file:
                        print(f"✓ Session summary saved to: {markdown_file}")
                        print("📝 Session summary ready for embedding on next system startup")
                    else:
                ...
                print("⚠ Warning: No LLM client provided for markdown summary generation")
    ...
    def generate_markdown_summary(self, session_entries: list, llm_client) -> str:
        ...
        try:
            ...
            if not llm_client or not hasattr(llm_client, 'client') or not llm_client.client:
                raise ValueError("LLM client is not properly initialized")
            
            print("  - Processing session data...")
            # Prepare session data for the LLM
            player_inputs = [entry for entry in session_entries if entry.get("entry_type") == "player_input"]
            dm_responses = [entry for entry in session_entries if entry.get("entry_type") == "dm_response"]
            dice_rolls = [entry for entry in session_entries if entry.get("entry_type") == "dice_roll"]
            
            print(f"  - Found {len(player_inputs)} player inputs, {len(dm_responses)} DM responses, {len(dice_rolls)} dice rolls")
            ...
            print(f"  - Created conversation flow with {len(conversation_flow)} entries")
...
Please generate a comprehensive but concise summary following the template above."""
            ...
            summary_content, response, was_streamed = llm_client.chat_completion_with_streaming(
                messages=messages,
            ...
            print(f"  - Successfully generated summary ({len(summary_content)} characters)")
    ...
    def _create_fallback_summary(self, session_entries: list) -> str:
        ...
        try:
            ...
            # Get session timing
            timestamps = [entry.get('timestamp', '') for entry in session_entries if entry.get('timestamp')]
...
"""
    ...
    def save_markdown_summary(self, session_id: str, markdown_content: str) -> str:
        ...
        try:
            ...
            existing_files = [f for f in os.listdir(sessions_dir) if f.startswith("session_") and f.endswith(".md")]
            session_numbers = []
            ...
            print(f"  - Successfully saved {len(markdown_content)} characters to {filepath}")
            return filepath
