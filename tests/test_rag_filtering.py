...
class TestRAGFiltering:
    ...
    def test_progression_filtering_blocks_future_content(self, context_retriever: ContextRetriever):
    ...
    def test_priority_scoring_boosts_current_act(self, context_retriever: ContextRetriever):
        ...
        new_distance = context_retriever._score_content_priority(test_result, 'Act I', 'Arc A')
        
        # Should be improved (lower distance is better)
        assert new_distance < test_result['distance'], \
            f"Current act content should get priority boost: {new_distance} vs {test_result['distance']}"
    
    def test_priority_scoring_penalizes_spoilers(self, context_retriever: ContextRetriever):
    ...
    def test_enhanced_query_intent_analysis(self, context_retriever: ContextRetriever):
        ...
        for case in test_cases:
            ...
            for expected_intent in case['expected_intents']:
                assert intent.get(expected_intent), \
...
class TestContentClassification:
    ...
    @pytest.mark.parametrize("act_number,current_act,should_allow", [
    ...
    def test_act_filtering_logic(self, context_retriever: ContextRetriever, 
                                act_number: str, current_act: int, should_allow: bool):
        ...
        if should_allow:
            assert len(filtered) == 1, f"Act {act_number} should be allowed for current act {current_act}"
        else:
            assert len(filtered) == 0, f"Act {act_number} should be blocked for current act {current_act}"
    ...
    def test_next_act_with_spoilers_blocked(self, context_retriever: ContextRetriever):
        ...
        filtered = context_retriever._filter_by_progression([test_result], 1)  # Current act 1
        assert len(filtered) == 0, "Next act content with spoilers should be blocked"
    
    def test_next_act_future_possibilities_blocked(self, context_retriever: ContextRetriever):
        ...
        filtered = context_retriever._filter_by_progression([test_result], 1)  # Current act 1
        assert len(filtered) == 0, "Next act future_possibilities content should be blocked"