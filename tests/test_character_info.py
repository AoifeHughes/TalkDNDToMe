...
class TestCharacterInfo:
    ...
    def test_character_name_retrieval(self, dm_engine: DMEngine):
        ...
        assert 'Rose' in response, f"Character name 'Rose' not found in: {response[:200]}..."
    ...
    def test_character_details_retrieval(self, dm_engine: DMEngine):
        ...
        for query in queries:
            ...
            character_keywords = ['elf', 'bard', 'charisma', 'level', 'stats', 'character', 'rose', 'ability', 'score']
            found_keywords = [kw for kw in character_keywords if kw.lower() in response.lower()]
            
            assert found_keywords, f"No character details found in response to '{query}': {response[:200]}..."
    ...
    def test_various_character_queries(self, dm_engine: DMEngine, query: str):
        ...
        assert 'error' not in response.lower(), f"Error in response to '{query}': {response}"
        assert 'trouble generating' not in response.lower(), f"LLM error in response to '{query}': {response}"