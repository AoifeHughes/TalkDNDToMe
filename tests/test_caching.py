...
class TestCacheManager:
    ...
    def test_file_cache_update_and_retrieval(self, cache_manager: CacheManager):
        ...
        # Update cache
        cache_manager.update_file_cache(test_file_path, test_hash, test_chunk_ids, test_metadata)
        
        # Retrieve cached chunks
        cached_chunks = cache_manager.get_cached_chunks(test_file_path)
        
        assert cached_chunks == test_chunk_ids, \
            f"Cached chunks don't match: expected {test_chunk_ids}, got {cached_chunks}"
    
    @pytest.mark.slow
    def test_cache_performance_improvement(self, cache_manager: CacheManager, 
                                         embedding_manager, dm_config):
...
class TestContentLoaderCaching:
    ...
    def test_content_loader_respects_cache(self, chroma_client, cache_manager, 
                                         embedding_manager, dm_config):
...
class TestCacheUtilities:
    ...
    def test_cache_handles_special_characters(self, cache_manager: CacheManager):
        ...
        assert cached_chunks == test_chunks, \
