...
class LLMClient:
    ...
    def chat_completion_stream(
        ...
        tools: Optional[List[Dict[str, Any]]] = None,
        tool_choice: str = "auto",
    ) -> Generator[str, None, Tuple[str, Any]]:
                    ...
                    if choice.finish_reason:
                        ...
                        class MockMessage:
                            def __init__(self, content, tool_calls):
                                ...
                                self.tool_calls = (
                                    [MockToolCall(tc) for tc in tool_calls]
                                    if tool_calls
                                    else None
                                )
                        ...
                        final_response = MockResponse(
                            [
                                MockChoice(
                                    complete_content, tool_calls, choice.finish_reason
                                )
                            ]
                        )
    ...
    def chat_completion_with_streaming(
        ...
        if (
            ...
            and self.config.streaming_fallback_on_tools
        ):
            use_streaming = False

        if use_streaming:
            try:
                stream_generator = self.chat_completion_stream(
